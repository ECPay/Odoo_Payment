...
ChoosePayment = {
    ...
    'ApplePay': 'ApplePay',  # ApplePay (若為PC版時不支援)
...
class CreateOrder(BasePayment):
    ...
    __INVOICE_EXTEND_PARAMETERS = {
        ...
        "CarrierType": {'type': str, 'required': False, 'max': 1},
        "CarrierNum": {'type': str, 'required': False, 'max': 64},
    ...
    def create_order(self, client_parameters):
        ...
        if client_parameters.get('InvoiceMark') == 'Y':
            ...
            if customer_identifier and client_parameters.get('CarrierType'):
                raise Exception(
                    'CarrierType do not fill any value, when CustomerIdentifier have value.')
            ...
            if print_param == '1':
                if not client_parameters.get('CustomerName') or not client_parameters.get('CustomerAddr'):
                    raise Exception('CustomerAddr have to fill value.')
                if not client_parameters.get('CarrierType'):
                    raise Exception('CCarrierType do not fill any value, when Print is "1".')
...
class OrderSearchPeriodic(BasePayment):
    ...
    def order_search_period(self, action_url=__url, client_parameters=None):
